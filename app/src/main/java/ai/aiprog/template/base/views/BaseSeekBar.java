...
import ai.aiprog.template.base.utils.BitmapUtil;
 ...
 */
public class BaseSeekBar<T extends Number> extends AppCompatImageView {
    /**
     * Default color of a {@link BaseSeekBar}, #FF33B5E5. This is also known as "Ice Cream Sandwich" blue.
     */
    ...
    private OnBaseSeekBarChangeListener<T> listener;
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        int defaultShadowYOffset = Dimensions.dpToPx(context, 2);
        int defaultShadowXOffset = Dimensions.dpToPx(context, 0);
        int defaultShadowBlur = Dimensions.dpToPx(context, 2);

        if (attrs == null) {
            setRangeToDefaultValues();
            internalPad = Dimensions.dpToPx(context, INITIAL_PADDING_IN_DP);
            barHeight = Dimensions.dpToPx(context, LINE_HEIGHT_IN_DP);
            ...
            TypedArray a = getContext().obtainStyledAttributes(attrs, R.styleable.BaseSeekBar, 0, 0);
            try {
                showTextAboveThumbs = a.getBoolean(R.styleable.BaseSeekBar_valuesAboveThumbs, true);
                setRangeValues(
                        extractNumericValueFromAttributes(a, R.styleable.BaseSeekBar_absoluteMinValue, DEFAULT_MINIMUM),
                        extractNumericValueFromAttributes(a, R.styleable.BaseSeekBar_absoluteMaxValue, DEFAULT_MAXIMUM),
                        extractNumericValueFromAttributes(a, R.styleable.BaseSeekBar_step, DEFAULT_STEP)
                );
                textAboveThumbsColor = a.getColor(R.styleable.BaseSeekBar_textAboveThumbsColor, Color.WHITE);
                singleThumb = a.getBoolean(R.styleable.BaseSeekBar_singleThumb, false);
                showLabels = a.getBoolean(R.styleable.BaseSeekBar_showLabels, true);
                internalPad = a.getDimensionPixelSize(R.styleable.BaseSeekBar_internalPadding, INITIAL_PADDING_IN_DP);
                barHeight = a.getDimensionPixelSize(R.styleable.BaseSeekBar_barHeight, LINE_HEIGHT_IN_DP);
                activeColor = a.getColor(R.styleable.BaseSeekBar_activeColor, ACTIVE_COLOR);
                defaultColor = a.getColor(R.styleable.BaseSeekBar_defaultColor, Color.GRAY);
                alwaysActive = a.getBoolean(R.styleable.BaseSeekBar_alwaysActive, false);

                Drawable normalDrawable = a.getDrawable(R.styleable.BaseSeekBar_thumbNormal);
                if (normalDrawable != null) {
                    thumbImage = BitmapUtil.drawableToBitmap(normalDrawable);
                }
                Drawable disabledDrawable = a.getDrawable(R.styleable.BaseSeekBar_thumbDisabled);
                if (disabledDrawable != null) {
                    thumbDisabledImage = BitmapUtil.drawableToBitmap(disabledDrawable);
                }
                Drawable pressedDrawable = a.getDrawable(R.styleable.BaseSeekBar_thumbPressed);
                if (pressedDrawable != null) {
                    thumbPressedImage = BitmapUtil.drawableToBitmap(pressedDrawable);
                }
                thumbShadow = a.getBoolean(R.styleable.BaseSeekBar_thumbShadow, false);
                thumbShadowColor = a.getColor(R.styleable.BaseSeekBar_thumbShadowColor, defaultShadowColor);
                thumbShadowXOffset = a.getDimensionPixelSize(R.styleable.BaseSeekBar_thumbShadowXOffset, defaultShadowXOffset);
                thumbShadowYOffset = a.getDimensionPixelSize(R.styleable.BaseSeekBar_thumbShadowYOffset, defaultShadowYOffset);
                thumbShadowBlur = a.getDimensionPixelSize(R.styleable.BaseSeekBar_thumbShadowBlur, defaultShadowBlur);

                activateOnDefaultValues = a.getBoolean(R.styleable.BaseSeekBar_activateOnDefaultValues, false);
            } finally {
                a.recycle();
            }
        }
        ...
        textSize = Dimensions.dpToPx(context, DEFAULT_TEXT_SIZE_IN_DP);
        distanceToTop = Dimensions.dpToPx(context, DEFAULT_TEXT_DISTANCE_TO_TOP_IN_DP);
        textOffset = !showTextAboveThumbs ? 0 : this.textSize + Dimensions.dpToPx(context,
        ...
        // make BaseSeekBar focusable. This solves focus handling issues in case EditText widgets are being used along with the BaseSeekBar within ScrollViews.
        setFocusable(true);
     ...
     */
    @SuppressWarnings("unused")
    public void setOnBaseSeekBarChangeListener(OnBaseSeekBarChangeListener<T> listener) {
...
//                    if (notifyWhileDragging && listener != null) {
                    if (listener != null) {
                        listener.onBaseSeekBarValuesChanged(this, getSelectedMinValue(), getSelectedMaxValue());
                    }
                }
                break;
    ...
    protected synchronized void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        int height = thumbImage.getHeight()
                + (!showTextAboveThumbs ? 0 : Dimensions.dpToPx(getContext(), HEIGHT_IN_DP))
    ...
    protected synchronized void onDraw(@NonNull Canvas canvas) {
        ...
        if (showTextAboveThumbs && (activateOnDefaultValues || !selectedValuesAreDefault)) {
            ...
            if (!singleThumb) {
                ...
                int spacing = Dimensions.dpToPx(getContext(), TEXT_LATERAL_PADDING_IN_DP);
    ...
    /**
     * Overridden to save instance state when device orientation changes. This method is called automatically if you assign an id to the BaseSeekBar widget using the {@link #setId(int)} method. Other members of this class than the normalized min and max values don't need to be saved.
     */
    @Override
    ...
    /**
     * Overridden to restore instance state when device orientation changes. This method is called automatically if you assign an id to the BaseSeekBar widget using the {@link #setId(int)} method.
     */
    @Override
     ...
     *
     * @param <T> The Number type the BaseSeekBar has been declared with.
     * @author Stephan Tittel (stephan.tittel@kom.tu-darmstadt.de)
     */
    public interface OnBaseSeekBarChangeListener<T extends Number> {

        void onBaseSeekBarValuesChanged(BaseSeekBar<T> bar, T minValue, T maxValue);
    }
